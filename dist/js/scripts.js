  ...
  $('.unit-img-list__el, .unit-img-list__el').click(function(){
	...
	function hiddenText () {
		let hiddenBtn = document.querySelector('.hidden__btn'),
			textHidden = document.querySelector('.text__hidden');
		if (hiddenBtn != null) {
	...
	function quesAnswer () {
		let questionContainer = document.querySelectorAll('.question__container');
		if (questionContainer[0] != undefined) {
			questionContainer[0].classList.add('question__container--active');
			for (let i = 0; questionContainer.length > i; i++) {			
				questionContainer[i].addEventListener('click', function () {
					questionContainer[i].classList.toggle('question__container--active');
				})
			}
		}		
	}
	...
	function quantity() {
		let basketQuantity = document.querySelectorAll('.basket__quantity');
		for (let i = 0; basketQuantity.length > i; i++) {
			let	basketQuantityBottom = basketQuantity[i].querySelector('.basket__quantity--bottom'),
				basketQuantityValue = basketQuantity[i].querySelector('.basket__quantity--value'),
				basketQuantityTop = basketQuantity[i].querySelector('.basket__quantity--top');

			basketQuantityBottom.addEventListener('click', function() {
				if (basketQuantityValue.value > 1) {
					basketQuantityValue.value--;
				}
			})
			basketQuantityTop.addEventListener('click', function() {				
				basketQuantityValue.value++;				
			})
		}		
	}
